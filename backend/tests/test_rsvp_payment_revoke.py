...
class TestHostRevokePayment:
    def test_revoke_regates_the_guest(self, api_client, auth_headers, test_user, django_user_model):
        ...
        assert rsvp.paid_revoked_at is not None
        assert rsvp.paid_confirmed_at is not None, "record of having paid must survive"
        assert rsvp.is_paid is False
    ...
    def test_guest_can_reconfirm_after_a_revoke(
        ...
        assert response.status_code == 200
        rsvp = EventRSVP.objects.get(event=event, user=guest)
        assert rsvp.is_paid is True
        assert rsvp.paid_revoked_at is None
    ...
    def test_host_can_confirm_via_patch(
        ...
        assert response.status_code == 200
        assert EventRSVP.objects.get(event=event, user=guest).is_paid is True
