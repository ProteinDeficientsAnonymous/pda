...
class TestSetAttendance:
    ...
    def test_marks_plus_one_independently_of_member(
        ...
        rsvp = EventRSVP.objects.create(
            event=open_check_in_event, user=members[0], status=RSVPStatus.ATTENDING, has_plus_one=True
        )
        url = f"/api/community/events/{open_check_in_event.id}/rsvps/{members[0].pk}/attendance/"
        api_client.post(
            url,
        ...
        api_client.post(
            url,
            {"attendance": AttendanceStatus.NO_SHOW, "for_plus_one": True},
            content_type="application/json",
            **_auth(host_user),
        )
        rsvp.refresh_from_db()
        assert rsvp.attendance == AttendanceStatus.ATTENDED
        assert rsvp.checked_in_at is not None
        assert rsvp.plus_one_attendance == AttendanceStatus.NO_SHOW
