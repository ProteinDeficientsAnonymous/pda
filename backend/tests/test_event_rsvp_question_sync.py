...
class TestEventRsvpQuestionSync:
    def test_host_can_atomically_replace_questions(self, api_client, auth_headers, rsvp_event):
        existing = _create_question(api_client, auth_headers, rsvp_event.id).json()
        removed = _create_question(
            api_client, auth_headers, rsvp_event.id, label="remove me"
        ).json()
    ...
    def test_replace_questions_rejects_stale_baseline(self, api_client, auth_headers, rsvp_event):
        existing = _create_question(api_client, auth_headers, rsvp_event.id).json()
        _create_question(api_client, auth_headers, rsvp_event.id, label="concurrent")
