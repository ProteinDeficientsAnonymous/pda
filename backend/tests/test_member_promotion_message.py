...
@pytest.fixture
def edit_member_message_user(db):
    ...
    role = Role.objects.create(
        name="member_message_editor", permissions=[PermissionKey.APPROVE_JOIN_REQUESTS]
    )
    user.roles.add(role)
    return user
...
@pytest.fixture
def edit_member_message_headers(edit_member_message_user):
    refresh = RefreshToken.for_user(edit_member_message_user)
...
class TestUpdateMemberPromotionMessage:
    def test_with_permission_updates_body(self, api_client, edit_member_message_headers):
        response = api_client.patch(
            "/api/community/member-promotion-message/",
            data={"body": "hi ${FIRST_NAME}, welcome to full membership!"},
            content_type="application/json",
            **edit_member_message_headers,
        )
        assert response.status_code == 200
    ...
    def test_empty_body_rejected(self, api_client, edit_member_message_headers):
        response = api_client.patch(
            "/api/community/member-promotion-message/",
            data={"body": "   "},
            content_type="application/json",
    ...
    def test_too_long_body_rejected(self, api_client, edit_member_message_headers):
        response = api_client.patch(
            "/api/community/member-promotion-message/",
            data={"body": "x" * 5000},
            content_type="application/json",
