...
class TestImageSearch:
    ...
    def test_gif_prefers_original_webp_when_smaller(self, api_client, auth_headers, settings):
        settings.GIPHY_API_KEY = "giphy-key"
        settings.PEXELS_API_KEY = ""
        payload = {
            "data": [
        ...
        with patch("community._giphy.httpx.get", side_effect=_by_url(gif_payload=payload)):
            response = api_client.get(f"{_URL}?q=party", **auth_headers)
        assert response.status_code == 200
        assert response.json()["results"][0]["original_url"] == "https://example.com/big.webp"

    def test_gif_keeps_downsized_large_when_webp_is_heavier(
