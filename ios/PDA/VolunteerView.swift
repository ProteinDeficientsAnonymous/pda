...
enum VolunteerEditorCopy {
    static let edit = "edit"
    static let done = "done"
    static let placeholder = "volunteer content"
    static let saving = "saving…"
    static let saved = "saved ✓"
    static let saveError = "couldn't save"
}

enum VolunteerAutosaveStatus: Equatable {
    case idle, saving, saved, error
}

func showsVolunteerEdit(_ user: SessionUser?) -> Bool {
    guard let user else { return false }
    return user.isAdmin || user.permissions.contains("edit_guidelines")
}

func volunteerAutosaveLabel(_ status: VolunteerAutosaveStatus) -> String? {
    switch status {
    case .idle: nil
    case .saving: VolunteerEditorCopy.saving
    case .saved: VolunteerEditorCopy.saved
    case .error: VolunteerEditorCopy.saveError
    }
}
...
@Observable
final class VolunteerEditorModel {
    var client: EventsClient
    var user: SessionUser?
    var page: HomePage?
    var editing = false
    var draft = ""
    var status: VolunteerAutosaveStatus = .idle
    var error: String?
    var autosaveDelay: Duration = .seconds(2)

    var canEdit: Bool { showsVolunteerEdit(user) }
    ...
    func load() async {
        error = nil
        do {
            page = try await client.volunteer()
        } catch {
            page = nil
    ...
    private func performSave(_ value: String) async {
        ...
        let task = Task { [weak self] in
            ...
            do {
                let page = try await self.client.updateVolunteer(contentPm: value)
                self.page = page
                self.status = .saved
...
struct VolunteerView: View {
    ...
    @State private var model: VolunteerEditorModel?
    ...
    var body: some View {
        NavigationStack {
            ...
            .task {
                if model == nil { model = VolunteerEditorModel(client: client, user: user) }
                await model?.load()
            }
        }
    }
    ...
    @ViewBuilder
    private func editor(_ model: VolunteerEditorModel) -> some View {
        VStack(alignment: .leading, spacing: 8) {
            if model.editing {
                HStack(spacing: 8) {
                    if let label = volunteerAutosaveLabel(model.status) {
                        Text(label)
                    ...
                    Spacer(minLength: 0)
                    PDAButton(VolunteerEditorCopy.done, variant: .ghost) {
                ...
                if model.canEdit {
                    HStack {
                        Spacer(minLength: 0)
                        PDAButton(VolunteerEditorCopy.edit, variant: .ghost) { model.beginEdit() }
                    }
                    .padding(.horizontal)
                }
    ...
    private func autosaveColor(_ status: VolunteerAutosaveStatus) -> Color {
        switch status {
    ...
    private func autosaveFill(_ status: VolunteerAutosaveStatus) -> Color {
        switch status {
